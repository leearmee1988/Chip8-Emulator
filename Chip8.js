...
Chip8.prototype = {
    ...
    loadRom: function(name) {
        ...
        request.onload = function () {
            if (request.response) {
                self.loadProgram(new Uint8Array(request.response));
            }
        }
    ...
    loadProgram: function(rom){
        for (var i = 0, length = rom.length; i < length; i++) {
    ...
    start: function(ctx){
        ...
        requestAnimationFrame(function me() {
            for (var i = 0; i <10; i++) {
                if(!self.paused){
                    console.log(self.pc);
                    console.log('fffffff: '+self.memory[512]);
                ...
                self.updateTimers();
                self.screen.render(ctx,self.display);
            ...
            case 0xD000:
                ...
                for (row = 0; row < height; row++) {
                    ...
                    for (col = 0; col < width; col++) {
                        if ((sprite & 0x80) > 0) {
                            if (this.screen.setPixel(this.v[x] + col, this.v[y] + row, this.display)) {
                                this.v[0xF] = 1;
                            }
                        }
